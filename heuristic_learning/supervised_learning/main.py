...
def sgdLearner(X, Y, batch_size, ita, training_epochs, num_test):
	...
	train_X, test_X = preprocessing(train_X, test_X)
	regr = linear_model.SGDRegressor(loss = 'huber', alpha = 0.01, verbose = 0, n_iter = 1000, fit_intercept = True)#, average=True)
	regr.fit(train_X, train_Y)
	print('Coefficients: \n', regr.coef_)
	# The mean squared error
	print("Mean squared error: %.2f"
...
def mlpRegressionLearner(X, Y, batch_size, ita, training_epochs, num_test):
	...
	regr = neural_network.MLPRegressor(hidden_layer_size=(20, ), activation = 'relu', solver='adam', alpha='0.0001', learning_rate='adaptive', learning_rate_init = 0.1, warm_start = True)
	regr.fit(train_X, train_Y)
	print('Coefficients: \n', regr.coef_)
	print("Mean squared error: %.2f"
      % np.mean((regr.predict(test_X) - test_Y) ** 2))
